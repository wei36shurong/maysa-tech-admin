...
const config = {
    ...
    output: {
        ...
        publicPath: "/", // 生成的公共目录
        filename: __DEV__ ? "js/index.js" : "js/[name].[hash].js", // 文件名,不加chunkhash,以方便调试时使用，生产环境下可以设置为 [name].bundle.[hash:8].js
        chunkFilename: "js/[id].[chunkhash].chunk.js" // 块文件索引
    },
    // 配置插件
    plugins: __DEV__ ? [].concat(plugins.commonPlugins).concat(plugins.devPlugins) : [].concat(plugins.commonPlugins).concat(plugins.prodPlugins),
    module: {
        rules: [
            loaders.jslint,
            // loaders.wasm,
